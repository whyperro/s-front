...
const MovementsByAccountPage = () => {
  ...
  const filteredData = data || []  // Filtrar solo las cuentas de esta caja
  ...
  const chartData = filteredData.map((account) => ({
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-xl font-semibold">Movimientos de la caja: {cashId}</h1>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <CardTitle className="text-center md:text-left">RESUMEN FINANCIERO</CardTitle>
              ...
              {filteredData.map((account) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="text-center text-green-700">{formatCurrency(income)}</TableCell>
                    <TableCell className="text-center text-red-700">{formatCurrency(outcome)}</TableCell>
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis tickFormatter={(value) => `$${value.toLocaleString("es-VE", { minimumFractionDigits: 0 })}`} />
                  <Tooltip
                  ...
                  />
                  <Legend />
                  <Bar dataKey="Ingresos" fill="#22c55e" name="Ingresos" onClick={handleBarClick} cursor="pointer" />
                  <Bar dataKey="Egresos" fill="#ef4444" name="Egresos" onClick={handleBarClick} cursor="pointer" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              <Card className="border border-muted-foreground/20 shadow-sm">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg">Movimientos de: {selectedAccount}</CardTitle>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                      <div className="bg-green-50 p-3 rounded-md">
                        ...
                        <p className="text-lg font-bold text-green-700">
                          {formatCurrency(Number(selectedAccountData.INCOME || 0))}
                        </p>
                      </div>
                      <div className="bg-red-50 p-3 rounded-md">
                        <p className="text-sm text-muted-foreground">Egresos</p>
                        <p className="text-lg font-bold text-red-700">
                          {formatCurrency(Number(selectedAccountData.OUTPUT || 0))}
                        </p>
                      </div>
                      <div
                        className={`p-3 rounded-md ${
                          Number(selectedAccountData.INCOME || 0) - Number(selectedAccountData.OUTPUT || 0) >= 0
                            ? "bg-green-50"
                            : "bg-red-50"
                        }`}
                      >
                        ...
                        >
                          {formatCurrency(
                            Number(selectedAccountData.INCOME || 0) - Number(selectedAccountData.OUTPUT || 0),
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    <TableBody>
                      {selectedAccountMovements.map((movement) => (
                        <TableRow key={movement.id}>
                          ...
                          <TableCell
                            className={
                              movement.type === "INCOME" ? "text-green-600 font-medium" : "text-red-600 font-medium"
                            }
                          >
                            {formatCurrency(movement.amount)}
                          </TableCell>
                          <TableCell>{movement.bank_account?.name || "Efectivo"}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
              ...
              <Card className="border border-muted-foreground/20 shadow-sm">
                ...
                <CardContent>
                  <div className="flex flex-col items-center justify-center py-8">
                    <p className="text-muted-foreground">No hay movimientos para esta cuenta</p>
                  </div>
                </CardContent>
              </Card>
            </div>
