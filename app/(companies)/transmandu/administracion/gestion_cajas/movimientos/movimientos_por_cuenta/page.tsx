...
const MovementsByAccountPage = () => {
  ...
  // Pass date parameters to the hook
  const { data, isLoading, isError } = useGetCashMovementByAccount({ from, to })
  ...
  const selectedAccountMovements = selectedAccount
    ? data?.find((account) => account.account_name === selectedAccount)?.movements || []
    : []
                    ...
                    <TableBody>
                      {selectedAccountMovements.map((movement) => (
                        <TableRow key={movement.id}>
                          <TableCell>{format(new Date(movement.date), "dd/MM/yyyy", { locale: es })}</TableCell>
